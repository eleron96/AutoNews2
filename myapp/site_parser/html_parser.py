...
def find_blog_posts(page_url):
    try:
        ...
        speckle_blog_posts = soup.find_all('div',
                                           class_='flex flex-col rounded-lg transition shadow hover:shadow-2xl overflow-hidden')
        ...
        for post in speckle_blog_posts:
            title_tag = post.find('p',
                                  class_='text-xl font-semibold text-gray-900 dark:text-gray-200')
            title = title_tag.get_text(
                strip=True) if title_tag else 'Без заголовка'

            description_tag = post.find('p',
                                        class_='mt-3 text-base text-gray-500 line-clamp-3 md:line-clamp-4')
            description = description_tag.get_text(
                strip=True) if description_tag else 'Без описания'

            link_tag = post.find('a', href=True)
            link = urljoin(page_url,
                           link_tag['href']) if link_tag else 'Без ссылки'
...
def display_blog_posts(blog_posts, start_date, end_date):
    ...
    for blog in blog_posts:
        try:
            ...
            published_date = datetime.strptime(blog['date'],
                                               '%d %b %Y').date() if blog[
                                                                         'date'] != 'Без даты' else datetime.today().date()
